...
function alertApplySuccess() {
    Swal.fire(
        "申请签到成功!", "", "success"
)
}
function alertCancelSuccess() {
    Swal.fire(
        "取消签到成功!", "", "success"
)
}
...
Calendar.prototype = {
    ...
    init: function () {
       ...
       // opts.width = el.width();
        ...
        el.on("click", ".calendar-cell", function (e) {
            ...
            $(this).addClass("hello");
            // $(this).css("border", "#")
            // $(this).children(":first").css("border", "2px solid #FF6633")
                                   // if(day == 1){
                                //     s += '<div class="calendar-sign">已签到</div>';
                                // } else if (day == 2) {
                                //     s += '<div class="calendar-replenish">待审批</div>';
                                // } else if (day == 3) {
                                //     s += '<div class="calendar-reject">拒批</div>';
                                // }
            var tipStr = '';
            var that = $(this);
            var status = $(this).children(":last").attr("status");
            var lastEl = $(this).children(":last");
            ...
            if (status === '11' || status === '13') {
                tipStr = '<div class="calendar-sign" status="12">已签到</div>';
                
                // that.find(":last-child").remove();
            }
            if (status === '12') {
                tipStr = '<div class="calendar-reject" status="13">拒批</div>';
	...
	calendardata:function(Y,weekDay){
        ...
        var C=function(m){  
            ...
            newarr.showInLunar = (newarr.showInLunar.length > 4) ? newarr.showInLunar.substr(0, 6) + "...": newarr.showInLunar ;
            if(holiday!=null){
    ...
    _refreshCalendar: function (newDate,cycleData) {
        ...
        me.structure(A,X);
        var ins= X.dateArray;
        renderDate.setDate(renderDate.getDate()+20);
        //固定六行
        for (var i = 0; i < 5; i++) {
            s += '<tr>'
            for (var l = 0; l < 7; l++) {
                ...
                // console.log(day);
                if (date > 20 && month-1 > now.getMonth()) {
                ...
                // }
                else if (month-1 == now.getMonth() && date == now.getDate()) {
	                ...
	                if(ins[a]!=null){
		                ...
		                if(ins[a].solarMonth==month ){
		                	if(ins[a].solarDate==date){
                                ...
                                // }
                                // console.log(day);
                                // else if(ins[a].holidayDate=="holiday"){
		                		// 	s += '<span class="calendar-holiday-bg"></span>';
		                		// }else if(ins[a].holidayDate=="work"){
		                			
		                		// 	s += '<span class="calendar-work-bg"></span>';
                                // }
                                continue;
		                	}
                        } 
                        // else if (ins[a].solarMonth == month + 1){
                        //     if(ins[a].solarDate==date){
                        //     s += '<div class="value">' + ins[a].showInLunar + '</div>';
                        //     }
                        // }
                ...
                s += '</div>';
                if (date === 10) {
                    s += '<div class="calendar-replenish" status="1">待审批</div>';
                }
                if (date === 17) {
                    s += '<div class="calendar-reject" status="2">拒批</div>';
                }
                if (date === 11) {
                    s += '<div class="calendar-replenish" status="11">待审批</div>';
                }
                s += '</td>'
